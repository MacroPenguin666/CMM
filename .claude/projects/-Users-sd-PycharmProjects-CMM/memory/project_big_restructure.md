...
3. **Distribution setup** — `data/` gitignored; `cmm.db` shared via Google Drive (link in README). `backend/bootstrap_db.py` lets users build fresh DB from scratch with their own API keys. README has quick-start section.
