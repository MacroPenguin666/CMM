...
def api_polity():
    try:
        from policy_monitor.polity import get_polity_data
...
def api_polity_meeting_news():
    try:
        from policy_monitor.polity import scrape_meeting_news
...
def api_eurostat_datasets():
    ...
    try:
        from policy_monitor.eurostat import get_eurostat_db, get_eurostat_datasets, DATASET_META
...
def api_eurostat_series():
    ...
    try:
        from policy_monitor.eurostat import get_eurostat_db, get_eurostat_series
...
def api_eurostat_latest():
    ...
    try:
        from policy_monitor.eurostat import get_eurostat_db, get_eurostat_latest
        dataset = request.args.get("dataset", "")
        if not dataset:
            return jsonify({"error": "dataset parameter required"}), 400
        conn = get_eurostat_db()
...
def api_eurostat_indicators():
    ...
    try:
        from policy_monitor.eurostat import get_eurostat_db, get_eurostat_indicators
        dataset = request.args.get("dataset", "")
        if not dataset:
            return jsonify({"error": "dataset parameter required"}), 400
        conn = get_eurostat_db()
...
def api_eurostat_sme_scorecard():
    ...
    """
    try:
        from policy_monitor.eurostat import get_eurostat_db, get_sme_scorecard
        conn = get_eurostat_db()
...
def api_eurostat_policy_scorecard():
    ...
    """
    try:
        from policy_monitor.eurostat import get_eurostat_db, get_policy_scorecard
        conn = get_eurostat_db()
...
def api_elites_congresses():
    try:
        from policy_monitor.ccp_elites import get_db, get_congresses, get_meta
        conn = get_db()
...
def api_elites_psc():
    try:
        from policy_monitor.ccp_elites import get_db, get_psc_by_congress
...
def api_elites_politburo():
    try:
        from policy_monitor.ccp_elites import get_db, get_pb_by_congress
...
def api_elites_cc():
    try:
        from policy_monitor.ccp_elites import get_db, get_cc_by_congress
        congress = request.args.get("congress") or None
        conn = get_db()
...
def api_elites_search():
    try:
        from policy_monitor.ccp_elites import get_db, search_person
...
def api_elites_purge_heatmap():
    ...
    try:
        import re
        from policy_monitor.ccp_elites import get_db
...
def api_trade_meta():
    ...
    try:
        from policy_monitor.unctad import get_unctad_db, get_available_years, get_hs2_codes, HS2_SECTIONS
        conn = get_unctad_db()
...
def api_trade_map():
    ...
    """
    try:
        from policy_monitor.unctad import get_unctad_db, get_trade_map
...
def api_trade_top():
    ...
    """
    try:
        from policy_monitor.unctad import get_unctad_db, get_top_partners
        year = request.args.get("year", type=int)
        flow = request.args.get("flow", "X")
        hs2  = request.args.get("hs2", "all")
...
def api_tariffs_china_applied():
    ...
    try:
        ...
        conn = sqlite3.connect(DB_DIR / "trade_stats.db")
        rows = conn.execute("""
