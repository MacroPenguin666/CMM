...
from policy_monitor.config import DESTATIS_TOKEN as _TOKEN, DESTATIS_BASE_URL as API_URL

log = logging.getLogger("destatis")

# GENESIS API supports public guest access — fall back to GAST if no token configured
_USERNAME = _TOKEN if _TOKEN else "GAST"

_HEADERS = {"Content-Type": "application/x-www-form-urlencoded"}


def _creds() -> dict:
    """Return username/password body params for every GENESIS request."""
    return {"username": _USERNAME, "password": ""}
...
def fetch_table_meta(table_id: str, language: str = "en") -> dict:
    body = {**_creds(), "name": table_id, "area": "all", "language": language}
    try:
