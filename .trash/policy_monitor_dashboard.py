...
from policy_monitor.regulations import (
...
def api_pipeline_status():
    ...
    flights_last = None
    try:
        from policy_monitor.flights import get_flights_db
    ...
    ships_last = None
    try:
        from policy_monitor.ships import get_ships_db
        sconn = get_ships_db()
...
<div class="panel" id="panel-regulations">
  <div style="max-width:1200px;margin:0 auto;">
    ...
    <div class="reg-subtabs">
      <div class="reg-sub active" data-regsub="active">Active Laws</div>
      <div class="reg-sub" data-regsub="reform">Under Reform</div>
    </div>

    <!-- Active Laws (MOFCOM) -->
    <div class="reg-panel active" id="reg-panel-active">
    ...
    <div class="reg-panel" id="reg-panel-reform">
      <div class="controls" style="margin-bottom:14px;">
  ...
  Object.keys(byLevel).sort((a,b)=>a-b).forEach(lvl => {
    ...
    Object.keys(topics).sort().forEach(topic => {
      ...
      html += '<div class="topic-docs"><table class="reg-table"><thead><tr>'
            ...
            + '</tr></thead><tbody>';
      rows.forEach(d => {
        const link = d.url ? '<a href="' + esc(d.url) + '" target="_blank">' + esc(d.title) + '</a>' : esc(d.title);
        html += '<tr><td>' + link + '</td>'
...
function renderNpc(bills) {
  ...
  Object.keys(byTopic).sort().forEach(topic => {
    ...
    byTopic[topic].forEach(b => {
      ...
      html += '<div class="bill-title">' + titleLink + '</div>';
...
document.querySelector('[data-panel="regulations"]').addEventListener('click', () => {
  loadMofcomDocs();
  if (document.querySelector('.reg-sub[data-regsub="reform"]').classList.contains('active')) loadNpcBills();
}, {once: true});
