...
Two-phase workflow:
  1. Discover all series once:   python -m backend.fetchers.nbs --discover
     Walks the NBS indicator tree and saves data/nbs_tree_cache.json.
  2. Fetch data:                 python -m backend.fetchers.nbs
     Reads the cache and pulls time-series data for every leaf indicator.

Usage:
    python -m backend.fetchers.nbs --discover         # walk tree, cache all series
    python -m backend.fetchers.nbs                    # fetch data (uses cache)
    python -m backend.fetchers.nbs --show             # show latest snapshots
    python -m backend.fetchers.nbs --show-series A01010101
"""
...
def main():
    ...
    if args.show:
        ...
        if not snaps:
            print("No NBS data yet. Run: python -m backend.fetchers.nbs --discover  then  python -m backend.fetchers.nbs")
            conn.close()
            return
